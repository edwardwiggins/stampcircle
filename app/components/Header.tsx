...
import { SlHome, SlPeople, SlBubbles, SlPieChart, SlBell, SlEnvolope } from "react-icons/sl";
...
const unreadNotificationsCount = useLiveQuery(
 ...
 ? db.social_user_notifications
  ...
  .and(notif => notif.notification_type !== 'new_message')
  .count()
 : 0,
 [userProfile]
);
 ...
 const unreadMessageCount = useLiveQuery(
  () => userProfile
    ? db.social_user_notifications
      .where({ receiving_user_id: userProfile.user_id, is_read: 0, notification_type: 'new_message' })
      .count()
    : 0,
  [userProfile]
 );
...
useEffect(() => {
 ...
 .on('broadcast', { event: 'new_notification' }, async ({ payload }) => { 
  ...
  const notificationsToStore = userNotifications.map((n: any) => ({ 
        ...n, 
        is_read: (n.notification_type === 'new_message' && isMessagesPageActive) ? 1 : 0 
      })); 
  await db.social_user_notifications.bulkPut(notificationsToStore);

      // --- THE FIX ---
      // Find any notifications that were just marked as read and sync this change to the server.
      const idsToMarkAsReadOnServer = notificationsToStore
      ...
      if (idsToMarkAsReadOnServer.length > 0) {
        await supabase
            ...
            .update({ is_read: true })
            .in('id', idsToMarkAsReadOnServer);
      }
      // --- END OF FIX ---

      const hasNewMessage = userNotifications.some(n => n.notification_type === 'new_message');
      if (hasNewMessage) {
 ...
 <header>
  ...
  >
   {unreadMessageCount === 0 ? (
        ...
        <SlEnvolope className={pathname === '/messages' ? 'icon-orange' : 'icon-red'} size={26} />
      )}
